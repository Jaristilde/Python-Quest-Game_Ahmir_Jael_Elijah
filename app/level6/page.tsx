...
export default function Level6Hub() {
    ...
    // Check if Level 5 is complete (13 lessons, stored as levels 63-75)
    const level5Complete = user.progress.completedLevels.filter(l => l.level >= 63 && l.level <= 75).length >= 13;
        ...
        return (
            <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
                <div style={{ textAlign: 'center' }}>
                    ...
                    <h1 style={{ fontSize: '1.75rem', fontWeight: 800, marginBottom: '0.75rem' }}>Level 6 is Locked!</h1>
                    <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem', maxWidth: '300px' }}>
                        Complete all 13 lessons in Level 5 to unlock Modules & APIs!
                    </p>
                    <Link href="/level5" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.875rem 1.5rem', background: 'linear-gradient(135deg, var(--accent-primary), var(--accent-secondary))', color: '#1E1E2E', textDecoration: 'none', borderRadius: '8px', fontWeight: 600 }}>
                        <Play size={18} /> Go to Level 5
                    </Link>
                </div>
            </div>
        );
    ...
    // Level 6 lessons are stored as levels 76-84
    const completedLessons = user.progress.completedLevels
        .filter(l => l.level >= 76 && l.level <= 84)
        .map(l => l.level - 75);
    ...
    const getBadgeStyle = (type: string) => {
        switch (type) {
            ...
            case 'supercharge':
                return { background: 'rgba(241, 250, 140, 0.15)', border: '1px solid rgba(241, 250, 140, 0.5)', color: 'var(--badge-supercharge)' };
            case 'project':
    ...
    const getBadgeText = (type: string) => {
        switch (type) {
            ...
            case 'supercharge': return '⚡ SUPERCHARGE';
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
            ...
            <div style={{ textAlign: 'center', padding: '2rem 1.5rem 1rem' }}>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    <h1 style={{ fontSize: '1.75rem', fontWeight: 800, marginBottom: '0.25rem' }}>Level 6: Modules & APIs</h1>
                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem' }}>Unlock Python's superpowers!</p>
                    ...
                    <div style={{ maxWidth: '300px', margin: '1.5rem auto 0' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                            <span>{completedLessons.length} of 9 complete</span>
                            <span>{Math.round((completedLessons.length / 9) * 100)}%</span>
                        </div>
                        <div style={{ height: '8px', background: 'var(--bg-card)', borderRadius: '9999px', overflow: 'hidden' }}>
                            <motion.div
                                ...
                                style={{ height: '100%', background: 'linear-gradient(90deg, #3b82f6, #8b5cf6)', borderRadius: '9999px' }}
                            />
                        </div>
                    </div>
                </motion.div>
                    ...
                    return (
                        ...
                        >
                            {isLocked ? (
                                ...
                                <Link
                                    href={`/level6/lesson${lesson.id}`}
                                    style={{
                                        display: 'flex',
                                        alignItems: 'center',
                                        padding: '1rem 1.25rem',
                                        background: isCurrent ? 'rgba(59, 130, 246, 0.1)' : 'var(--bg-card)',
                                        borderBottom: '1px solid var(--border-default)',
                                    ...
                                    onMouseLeave={(e) => {
                                        e.currentTarget.style.background = isCurrent ? 'rgba(59, 130, 246, 0.1)' : 'var(--bg-card)';
                                    }}
                                >
                                    <div style={{ width: '40px', display: 'flex', alignItems: 'center' }}>
                                        {isCompleted ? (
                                            ...
                                            <span style={{ fontSize: '0.9rem', color: isCurrent ? '#3b82f6' : 'var(--text-secondary)', fontWeight: 600 }}>
                                                {String(lesson.id).padStart(2, '0')}
                                            </span>
                                        )}
                                    </div>
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '0.15rem' }}>{lesson.description}</div>
                                    </div>
