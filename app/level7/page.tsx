...
export default function Level7Hub() {
    ...
    // Check if Level 6 is complete (9 lessons, stored as levels 76-84)
    const level6Complete = user.progress.completedLevels.filter(l => l.level >= 76 && l.level <= 84).length >= 9;
        ...
        return (
            <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
                <div style={{ textAlign: 'center' }}>
                    ...
                    <h1 style={{ fontSize: '1.75rem', fontWeight: 800, marginBottom: '0.75rem' }}>Level 7 is Locked!</h1>
                    <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem', maxWidth: '300px' }}>
                        Complete all 9 lessons in Level 6 to unlock Text Wizardry!
                    </p>
                    <Link href="/level6" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.875rem 1.5rem', background: 'linear-gradient(135deg, #3b82f6, #8b5cf6)', color: 'white', textDecoration: 'none', borderRadius: '8px', fontWeight: 600 }}>
                        <Play size={18} /> Go to Level 6
                    </Link>
                </div>
            </div>
        );
    ...
    // Level 7 lessons are stored as levels 85-96
    const completedLessons = user.progress.completedLevels
        .filter(l => l.level >= 85 && l.level <= 96)
        .map(l => l.level - 84);
    ...
    const getBadgeStyle = (type: string) => {
        switch (type) {
            ...
            case 'quiz':
                return { background: 'rgba(139, 233, 253, 0.15)', border: '1px solid rgba(139, 233, 253, 0.5)', color: '#8be9fd' };
            default:
        ...
        <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
            ...
            <div style={{ textAlign: 'center', padding: '2rem 1.5rem 1rem' }}>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    <h1 style={{ fontSize: '1.75rem', fontWeight: 800, marginBottom: '0.25rem' }}>Level 7: Text Wizardry</h1>
                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.95rem' }}>Master strings & list operations!</p>
                    ...
                    <div style={{ maxWidth: '300px', margin: '1.5rem auto 0' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                            ...
                            <span>{Math.round((completedLessons.length / 12) * 100)}%</span>
                        </div>
                        <div style={{ height: '8px', background: 'var(--bg-card)', borderRadius: '9999px', overflow: 'hidden' }}>
                            <motion.div
                                ...
                                style={{ height: '100%', background: 'linear-gradient(90deg, #a855f7, #ec4899)', borderRadius: '9999px' }}
                            />
                        </div>
                    </div>
                </motion.div>
                    ...
                    return (
                        ...
                        >
                            {isLocked ? (
                                ...
                                <Link
                                    href={`/level7/lesson${lesson.id}`}
                                    style={{
                                        display: 'flex',
                                        alignItems: 'center',
                                        padding: '1rem 1.25rem',
                                        background: isCurrent ? 'rgba(168, 85, 247, 0.1)' : 'var(--bg-card)',
                                        borderBottom: '1px solid var(--border-default)',
                                        borderLeft: isCurrent ? '3px solid #a855f7' : '3px solid transparent',
                                        textDecoration: 'none',
                                        color: 'inherit',
                                        transition: 'all 0.2s',
                                        boxShadow: isCurrent ? '0 0 15px rgba(168, 85, 247, 0.2)' : 'none'
                                    }}
                                    onMouseEnter={(e) => {
                                        e.currentTarget.style.background = 'var(--bg-card-hover)';
                                    }}
                                    onMouseLeave={(e) => {
                                        e.currentTarget.style.background = isCurrent ? 'rgba(168, 85, 247, 0.1)' : 'var(--bg-card)';
                                    }}
                                >
                                    <div style={{ width: '40px', display: 'flex', alignItems: 'center' }}>
                                        {isCompleted ? (
                                            ...
                                            <span style={{ fontSize: '0.9rem', color: isCurrent ? '#a855f7' : 'var(--text-secondary)', fontWeight: 600 }}>
                                                {String(lesson.id).padStart(2, '0')}
                                            </span>
                                        )}
                                    </div>
